 ...
 */
@Component
public class AuthorisationTokenFilter extends AbstractAuthenticationProcessingFilter
{
    private Logger LOG = LoggerFactory.getLogger(AuthorisationTokenFilter.class);

    @Autowired
    private TokenUtils tokenUtils;

    public AuthorisationTokenFilter()
    {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException, ServletException
    {
        ...
        if(tokenUtils.exist(authorisation))
        {
            ...
            return new UserAuthentication(authority,login,password);
        }
        else
        {
            LOG.info("Authorisation is Not Valid");
